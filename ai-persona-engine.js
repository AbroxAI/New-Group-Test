// ====================== AI PERSONA ENGINE v8.0 (Final – 150 Custom Personas) ======================
// 100 real named avatars + 50 fallback placeholders · Tuned for realism · All phrase banks included
// =================================================================================================

(function(){
  "use strict";

  // ---------- CONFIGURATION (fine‑tuned for natural chat flow) ----------
  const CONFIG = {
    BASE_INTERVAL: 8000,                // Slightly slower base interval
    BURST_CHANCE: 0.20,                 // Fewer bursts, more natural
    TRADE_RESULT_INTERVAL: 20000,       // Results every ~20 sec
    TRADE_RESULT_CHANCE: 0.40,
    TESTIMONIAL_CHANCE: 0.12,
    JOIN_CHANCE: 0.06,
    MAX_BURST_MESSAGES: 4,
    ENABLE_LOGGING: true,
    WATCHER_ACTIVITY_PENALTY: 0.65,     // Lurkers even quieter
    REPLY_CHANCE: 0.95                  // Almost always reply, but not 100%
  };
  ...
  // ---------- AVATAR SYSTEM (Safe filenames + fallback placeholders) ----------
  ...
  const customPersonas = [
    // 100 real names (personalities assigned via mapping below)
  ...
  // Map real names to personality presets (fine‑tuned assignments)
  ...
  // ---------- BUILD PERSONAS WITH TUNED TRAITS ----------
  const personas = [];
  let idCounter = 1;
  ...
  customPersonas.forEach((p, index) => {
    ...
    // Tuned typing speeds (words per minute * 60 = ms per char ~ 60-200ms)
    let typingBase;
    if (personality.experience === 'beginner') typingBase = [1800, 3500]; // slower
    else if (personality.experience === 'intermediate') typingBase = [900, 1800];
    else typingBase = [500, 1000]; // experts type fast

    // Grammar & slang fine‑tuned
    ...
    // Add personality‑specific tweaks
  ...
  // ---------- FULL PHRASE BANKS (merged from original) ----------
  ...
  log(`🤖 AI Persona Engine v8.0 loaded with ${personas.length} custom personas.`);
})();
