// ====================== AI PERSONA ENGINE v19 (REACTIONS + SOUNDS) ======================
// Reactions added: delayed emoji on selected messages. Chat sound via Web Audio.
// All previous features intact. No duplicate join messages.
// ========================================================================================
...
(function(){
  ...
  const CONFIG = {
    ...
    // REACTION SETTINGS
    REACTION_CHANCE: 0.3,            // base chance a message gets a reaction
    FUNNY_IMPORTANT_CHANCE: 0.6,     // higher chance for funny / testimonial / result / hype
    REACTION_MIN_DELAY: 3000,        // no reaction before 3 sec
    REACTION_MAX_DELAY: 15000,       // reaction within 15 sec
  };
  ...
  const globalPhraseBank = {
    question: [
      ...
      "do you use any EA?", "what's your favorite indicator combination?", "how to deal with FOMO?", "do you trade major news like NFP?",
      "what's your risk per trade in dollars?", "how to calculate lot size automatically?", "do you use pending orders?",
      "what's your longest win streak?", "how do you handle a losing streak?"
    ],
    result: [
      ...
      "today's lesson: don't force trades", "staying disciplined paid off", "printed pips all morning"
    ],
    reaction: [
    ...
    funny: [
      ...
      "my wife thinks I'm gambling, but it's called 'technical analysis'", "I'm not losing, I'm just paying for market education",
      "my luck ran out the moment I opened a live account"
    ],
    analytical: [
  ...
  const REACTION_EMOJI_MAP = {
    ...
    [MessageType.COMMUNITY]: ['🤝','❤️'],
    [MessageType.FLEX]: ['😎','💸'],
    [MessageType.CONFUSED]: ['🤷','😵'],
    [MessageType.SARCASTIC]: ['😏','🙃'],
    [MessageType.REACTION]: ['👍','❤️'],
    default: ['👍','❤️','🔥']
  };

  function pickReactionEmoji(type) {
    const list = REACTION_EMOJI_MAP[type] || REACTION_EMOJI_MAP.default;
    return pick(list);
  }
  ...
  // ---------- REACTION SCHEDULING ----------
  function maybeScheduleReaction(msgId, messageType) {
    if (!msgId) return;
    if (Math.random() > CONFIG.REACTION_CHANCE) return;
    if (messageType === MessageType.FUNNY || messageType === MessageType.TESTIMONIAL ||
        messageType === MessageType.RESULT || messageType === MessageType.HYPE) {
      if (Math.random() < CONFIG.FUNNY_IMPORTANT_CHANCE) {
        scheduleReaction(msgId, messageType);
        return;
      }
    }
    if (Math.random() < 0.2) scheduleReaction(msgId, messageType);
  }

  function scheduleReaction(msgId, msgType) {
    const delay = randomBetween(CONFIG.REACTION_MIN_DELAY, CONFIG.REACTION_MAX_DELAY);
    scheduleTimeout(() => {
      if (!simulationActive || !isGeneralChatActive()) return;
      const reactor = pickDifferentPersona();
      if (!reactor) return;
      const emoji = pickReactionEmoji(msgType);
      const api = getChatAPI();
      if (api.addReactionToMessage) {
        api.addReactionToMessage(msgId, reactor.name, emoji);
      }
    }, delay);
  }
  ...
  async function sendPersonaMessageOriginal(persona, replyTo=null, forceNoMedia = false){
    ...
    scheduleTimeout(() => {
      ...
      if(api.addIncomingMessage){
        msgId = api.addIncomingMessage(msgData);
        if (msgId) {
          maybeScheduleReaction(msgId, type);
          playChatSound();
        }
  ...
  function forceReplyToLastAIMessage() {
    ...
    scheduleTimeout(() => {
      hideTyping();
      if(api.addIncomingMessage) {
        const msgId = api.addIncomingMessage(msgData);
        if (msgId) {
          maybeScheduleReaction(msgId, MessageType.REACTION);
          playChatSound();
        }
  ...
  function simulateJoin(){
    ...
    const now = new Date(); const timeStr = now.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    const api = getChatAPI();
    if(api.addSystemMessage) {
      api.addSystemMessage({ text: `🎉 ${p.name} ${joinText}`, time: timeStr });
      playChatSound();
    }
  }
  ...
  function injectTradeResult(){
    ...
    if(Math.random() > 0.5){
      ...
      if(p && !(p.archetype === 'watcher' && Math.random() > 0.2)){
        ...
        const now = new Date(); const timeStr = now.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
        if(api.addIncomingMessage) {
          const msgId = api.addIncomingMessage({ senderName: p.name, senderAvatar: p.avatar, text, time: timeStr, personaId: p.id, messageType: MessageType.RESULT });
          if (msgId) {
            maybeScheduleReaction(msgId, MessageType.RESULT);
            playChatSound();
          }
    ...
    const now = new Date(); const timeStr = now.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    if(api.addSystemMessage) {
      api.addSystemMessage({ text, time: timeStr });
      playChatSound();
    }
  }
  ...
  log(`🤖 AI Persona Engine v19 – reactions & sounds enabled.`);
})();
