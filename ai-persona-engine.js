// ====================== AI PERSONA ENGINE v7.3 (Full with message IDs & parentId) ======================
// 450+ personas · Realistic avatars · Archetypes · Forced AI-to-AI replies with threading
...
(function(){
  ...
  const CONFIG = {
    BASE_INTERVAL: 7000,
    BURST_CHANCE: 0.25,
    TRADE_RESULT_INTERVAL: 18000,
    TRADE_RESULT_CHANCE: 0.45,
  ...
  // Helper to get the last AI message (excluding a given personaId) and return full object with id and text
  function getLastReplyTarget(excludePersonaId = null) {
    const target = [...recentMessages].reverse().find(m => m.personaId !== 'user' && m.personaId !== excludePersonaId);
  ...
  // Original sendPersonaMessage (to be used internally) – now with id and parentId
  function sendPersonaMessageOriginal(persona, replyTo=null){
    ...
    // Attach replyTo if provided or get from recent messages
    let finalReplyTo = replyTo;
    if(!finalReplyTo) {
      const lastTarget = getLastReplyTarget(persona.id);
      if(lastTarget) finalReplyTo = lastTarget;
    }
    if(finalReplyTo) {
      msgData.parentId = finalReplyTo.id;
      msgData.replyTo = {
        messageId: finalReplyTo.id,
        senderName: finalReplyTo.senderName,
        text: finalReplyTo.text
      };
    }
  ...
  function forceReplyToLastAIMessage() {
    ...
    if(!persona) return;
    const target = getLastReplyTarget(persona.id);
    if(!target) return;
    ...
    lastPersonaId = persona.id;
    log(`🤖 ${persona.name} forced reply to AI ${target.senderName} with preview: "${replyText}"`);
  }
  ...
  log(`🤖 AI Persona Engine v7.3 loaded with ${personas.length} personas. Forced AI-to-AI replies with message IDs and parentId. Reply preview bar always used. Chat seeded with initial AI messages.`);
})();
